...
export async function loadDocumentsFromRaw(relativePath: string): Promise<Document[]> {
  ...
  // 采集文件时间，补充到元数据（ISO 格式）。
  let stat: Stats | undefined
  try {
    stat = (await fs.stat(abs)) as Stats
  } catch (e) {
    // 忽略 stat 失败，不阻断加载
    stat = undefined
  }
...
export async function buildChromaRetriever(
  ...
  const retriever = store.asRetriever({
    ...
    searchKwargs:
      searchType === 'mmr'
        ? ({ lambda: mmrLambda, fetchK, filter: options.where, where: options.where } as any)
        : ({ filter: options.where, where: options.where } as any)
  })
  return retriever
}
